    ...
    for (int i = 0; i < number_of_operations; i++) {
        ...
        if (Сommands[i][0] == "NEXT") {
            ...
            if (DaysInMonth[next_month] > DaysInMonth[next_month - 1]) {
                for (int j = DaysInMonth[next_month - 1]; j < DaysInMonth[next_month] + 1; j++) {
            ...
            if (DaysInMonth[next_month] < DaysInMonth[next_month - 1]) {
                ...
                for (int j = 0; j < DaysInMonth[next_month - 1] - DaysInMonth[next_month]; j++) {
                    if (Days[DaysInMonth[next_month] + j].size() != 0) {
                        Days1[Days1.size() - 1].insert(end(Days1[Days1.size() - 1]), begin(Days[DaysInMonth[next_month] + j]), end(Days[DaysInMonth[next_month] + j]));
                    }
                }
                Days = Days1;
            }
